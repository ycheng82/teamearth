...
int main( int argc, char *argv[] )
{
	...
	//
	//	std::cout<<"GradientMagnitudeRecursiveGaussianImageFilter "<<"started";
	//	timer.Start("GradientMagnitudeRecursiveGaussianImageFilter");
	watershedFilter->SetInput( gradienMagnitudeFilter->GetOutput() );
	//	timer.Stop("GradientMagnitudeRecursiveGaussianImageFilter");
	...
	//	std::cout<<"watershedFilter "<<"started";
	//	timer.Start("watershedFilter");
	colorMapFilter->SetInput(  watershedFilter->GetOutput() );
	//	timer.Stop("watershedFilter");
	
	//	std::cout<<"colorMapFilter "<<"started";
	//	timer.Start("colorMapFilter");
	writer->SetInput( colorMapFilter->GetOutput() );
	//	timer.Stop("colorMapFilter");
	
	try
	{
		std::cout<<"Pipeline execution started "<< "threshold: " << argv[3] << " level: " << argv[4] <<std::endl;
		timer.Start("watershedFilter");
		writer->Update();
		timer.Stop("watershedFilter");
		std::cout<<"Pipeline execution ended"<<std::endl;
